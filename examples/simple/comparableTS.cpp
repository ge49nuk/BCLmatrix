...
double stealPerc, throughputTime;
bool verifyResults = false, printThroughput = true, throttled = false;
...
double execute(int tasks, int taskStealing, int *migratedTasks)
{
    ...
    tasksStolen = 0, tasksExecuted = 0, throughputTime = 0;
    ...
    for (size_t rank = 0; rank < BCL::nprocs(); rank++)
    {
        ...
        resultQueues.push_back(BCL::CircularQueue<taskRes>(rank, 2400));
    }
...
bool steal(std::vector<BCL::CircularQueue<taskNoRes>> *queues)
{
    ...
    for (std::vector<int>::iterator it = ranks.begin(); it != ranks.end(); ++it)
    {
        ...
        {
            double throughPutStart = curtime();
            if ((*queues)[*it].pop(t))
            {
                throughputTime += curtime() - throughPutStart;
                (*queues)[BCL::rank()].push(t);
                tasksStolen++;
            }
            else
...
int main(int argc, char **argv)
{
    ...
    double tsRuntime = 0, noTsRuntime = 0, throughputTimeTotal = 0;
    int migratedTasksTotal = 0, stolenTasks = 0;
    for (int i = 0; i < iterations; i++)
    {
        if (stealing > 0){
            tsRuntime += execute(tasks, 1, &migratedTasksTotal);
            throughputTimeTotal += throughputTime;
            stolenTasks += tasksStolen;
        }

        if (stealing == 0 || stealing == 2)
            noTsRuntime += execute(tasks, 0, &migratedTasksTotal);

    }
    tsRuntime /= iterations;
    noTsRuntime /= iterations;
    migratedTasksTotal /= iterations;
    throughputTimeTotal /= iterations;
    ...
    if (BCL::rank() == 0)
    {
        ...
        double Mbytes = ((long)stolenTasks * sizeof(taskNoRes))/1000000;
        if(printThroughput && stolenTasks != 0)
            printf("Average throughput: %lf\n", Mbytes / throughputTimeTotal);
    }
    BCL::finalize();
}
